...
var messages_en_US = messageMap{
	...
	CLIMessages_LastSessionFilePathDescription:              `Directory where the server stores last session connection details for reconnection. Defaults to an OS-specific cache directory. Can be set to ${workspaceFolder} in VS Code mcp.json for per-project session persistence.`,
	CLIMessages_TryToAdoptDescription:                       `When set to true alongside --use-last-session, the server will attempt to adopt orphaned MATLAB sessions from previous VS Code windows that are no longer running. Defaults to false.`,
	...
	CLIMessages_UseLastSessionDescription:                   `When set to true, the server attempts to reconnect to a previously started MATLAB session on startup by reading saved connection details. If reconnection fails, a new session is started. Defaults to false.`,
